...
const SemesterRow = ({
  ...
  const handleSearchInputChange = (e) => {
    ...
    const filteredResults = catalogData.filter((course) =>
      course.Course.Title.toLowerCase().includes(inputValue.toLowerCase())
    );
    setSearchResults(filteredResults);
  };

  useEffect(() => {
    //useEffect is to update the table row data after a course is removed
    setSelectedCourse({
      CourseCode: tableData[index].CourseCode,
      Title: tableData[index].CourseTitle,
      Credits: tableData[index].Credits,
  ...
  const handleCourseSelection = (selectedCourse) => {
    ...
    setSaveData((prevData) => {
      ...
      newData[semIndex].Courses[index] = {
        CourseCode: selectedCourse.Course.CourseCode,
        CourseTitle: selectedCourse.Course.Title,
        Credits: selectedCourse.Course.Credits,
        Grade: "",
  ...
  return (
    <tr key={index}>
      ...
      <td>{selectedCourse.Credits !== "" ? selectedCourse.Credits : ""}</td>
      <td>
...
const SemesterBody = ({
  ...
  const handleAddRow = () => {
    setTableData([
      ...tableData,
      { CourseCode: "", CourseTitle: "", Credits: "", Grade: "" },
    ]);
    const newSemesterData = {
      ...
      Courses: [
        ...tableData,
        { CourseCode: "", CourseTitle: "", Credits: "", Grade: "" },
      ], // Copy the tableData into the Courses array
    };
    setSaveData((prevData) => {
      const newData = [...prevData];
  ...
  const handleRemoveRow = (indexToRemove) => {
    ...
    setSaveData((prevData) => {
      ...
      newData[semIndex] = {
        ...prevData[semIndex],
        Courses: prevData[semIndex].Courses.filter(
  ...
  return (
    <tbody>
      ...
      <tr>
        <td colSpan="4">
          <button className="btn btn-accent w-full" onClick={handleAddRow}>
            <FaPlus />
          </button>
        </td>
      </tr>
...
const WhatIfExtra = ({
  ...
  const extraSemIndex = extraSemester.findIndex(
    (item) => item[0] === semNumber
  );
  ...
  const updateSemesterGPAs = () => {
    ...
    return (totalPoints / totalCredits).toFixed(2) !== "NaN"
  ...
  useEffect(() => {
    setCurrentGPAs((prevGPAs) => {
      ...
      const index = extraSemester.findIndex((item) => item[0] === semNumber);
      if (
        calculateGPA(userCourses) !== null &&
        calculateGPA(userCourses) !== "0.00"
      ) {
        newGPAs[index] = calculateGPA(userCourses);
        return newGPAs;
      } else if (
        updateSemesterGPAs() !== null &&
        updateSemesterGPAs() !== "0.00"
      ) {
  ...
  return (
    <>
      <div className="rounded-lg shadow px-3 pt-2 pb-4 bg-base-200">
        <h1
          className="tooltip pb-2 pl-1 pr-2 text-2xl font-bold text-center bg-base-200 rounded-t-lg flex gap-3 items-center"
          data-tip={
            currentGPAs[extraSemIndex] !== null &&
            currentGPAs[extraSemIndex] !== 0
              ? currentGPAs[extraSemIndex]
              : "No grade"
          }
        >
          <select
