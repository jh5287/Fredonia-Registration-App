...
import {
  ...
  getCustomList,
  getCustomSems,
} from "@/firebase/firebaseManagement";
import Icon from "@mdi/react";
import { mdiProgressHelper } from "@mdi/js";
import { cn } from "@/lib/utils";

const RegSemester = ({ number, data, updateRealStudentData }) => {
  ...
  return (
    <div className="rounded-lg shadow px-3 pt-2 pb-4 bg-base-200">
      <div className="flex justify-between items-center">
        <h1
          className="py-2 pl-1 text-lg font-semibold"
          data-tip={
            calculateGPA(data) !== null && calculateGPA(data) !== 0
              ? calculateGPA(data)
              : "No grade"
          }
        >
    ...
    <div>
      <div className="stat py-0 ">
        ...
        <div className="stat-value">
          {newCGPA ? newCGPA.toFixed(2) : "Unavailable"}{" "}
        </div>
        <div className="stat-desc">
          Edit grades to see how your CGPA will change!
        </div>
      </div>
    </div>
  );
...
const FuturePlan = () => {
  ...
  const fetchUserCourses = async () => {
    try {
      ...
      const uniqueTerms = Array.from(
        new Set(data.map((item) => `${item.Term.Semester} ${item.Term.Year}`))
      );
      ...
      // Add "modified grade" field to handle grade changes
      const dataWithModifiedGradeField = organized_data.map((term) =>
        term.map((courseRecord) => ({
          ...courseRecord,
          modifiedGrade: null,
        }))
      );

      setRealStudentData(dataWithModifiedGradeField);
    } catch (error) {
  ...
  useEffect(() => {
    //this useEffect is to update the newCGPA whenever the currentGPAs or extraSemGPAs change
  ...
  const updateRealStudentData = (index, courseRecord) => {
    if (index >= 0 && index < realStudentData.length) {
      setRealStudentData((prevData) => {
        const updatedStudentData = [...prevData];
        updatedStudentData[index] = courseRecord;
    ...
    <>
      <div className="p-3 pt-6">
        ...
        <div className="flex justify-center">
          ...
          <select
            value={selectedList}
            className="select input-bordered m-5"
          ...
          {extraSemester.map((item, index) => {
            return (
              <div key={item[0]} className="relative">
                ...
                />
                <button
                  className="btn btn-error btn-circle btn-xs absolute right-2 top-2 text-white"
                  onClick={() => removeExtraSemester(index)}
                >
                  <GoDash />
                </button>
              </div>
        ...
        <div className="grid grid-cols-1 gap-8 h-full lg:grid-cols-2">
          {realStudentData.map((item, index) => (
            <RegSemester
              ...
              number={index}
              data={item}
              updateRealStudentData={updateRealStudentData}
            /> //regSemester was moved out of components folder and is local to this page to include the grade change combo boxes
          ))}
        </div>
      </div>
    </>
